...
description: Bidirectional whiteboard brainstorming on an Excalidraw canvas. Use when the user invokes `whiteboard-brainstorm` explicitly or asks to brainstorm visually, explore ideas on a canvas, or mindmap.
---
...
Wrapper entrypoints may expose this as:

- Claude Code: `/whiteboard-brainstorm [--manual] <mode> [topic]`
- Codex: `whiteboard-brainstorm [--manual] <mode> [topic]`

Modes: `preimpl`, `general`, `mindmap`.

**Default behavior is auto-poll:** after each turn, the assistant issues a
blocking shell command that polls `events.jsonl` for the next ping
...
2. Start the server on the session dir:
   ...
   **Codex foreground fallback:** if the host reaps background processes
   after shell commands return, start the server as a persistent foreground
   command instead:
   ```
   node <plugin>/server/server.cjs --session-dir <sessionDir>
   ```
   Keep that command session open, then read `<sessionDir>/.state/server-info`
   from a second command to get the URL. Use this fallback whenever
   `start-board-server.sh` returns a URL but `/health` refuses the connection.

3. If `--manual` was NOT passed, do not stop after sharing the URL.
   Immediately enter the Auto-poll wait described below. When a ping
   arrives, run the turn loop. This is required: starting the server alone
   does not produce assistant output on the whiteboard.

4. If `--manual` was passed, stop after sharing the URL and wait for the
   user to return to the terminal.
...
10. **Reply in terminal** with a one-line summary of the shapes you
    ...
    when ready. After replying, immediately enter the Auto-poll wait again
...
### Legacy loop wrapper (rarely needed now)

Some hosts may expose loop wrappers or scheduled wakeups. The blocking shell
loop above supersedes that for almost all cases. Use the plain
`whiteboard-brainstorm <mode>` entrypoint for a single brainstorm session.
...
| `@rewrite` on an AI element | The assistant replaces that element next turn. |
...
If the wrapper entrypoint is invoked as `resume <slug>`:
