...
export async function exportPng({ rootArg, slug, outPath }) {
  ...
  return exportSceneToPng({ sceneFile, outPath: outPath || join(dir, 'latest.png') });
}

// Low-level variant: renders any scene file to a PNG at outPath. Used by
// the server thumbnail endpoint which works off versioned scene files.
export async function exportSceneToPng({ sceneFile, outPath }) {
