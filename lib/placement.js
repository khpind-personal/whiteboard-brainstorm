...
export function placeNear(target, blockers, size = { width: 200, height: 80 }) {
  // Stay in the target's column — shift DOWN when blocked rather than
  // fall back to a grid scan that can push elements past the viewport.
  // Only give up after a much more generous shift budget.
  const MAX_SHIFTS = 100;
  const SHIFT_DOWN = size.height + 20;
  const x = target.x + target.width + GAP;
  let y = target.y + GAP;
