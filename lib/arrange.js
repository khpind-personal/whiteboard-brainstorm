...
// Atomic turn allocator. Opens board-v<N>.excalidraw.json with O_EXCL so two
// concurrent arranges / sweeps can't both land on the same N and clobber.
// Returns { turn, path } with the file already present (empty). Caller then
// overwrites with the real scene.
export function allocateTurn(dir) {
  ...
  for (let attempt = 0; attempt < 50; attempt++) {
    const path = join(dir, `board-v${n}.excalidraw.json`);
    try {
  ...
  throw new Error('allocateTurn exhausted 50 attempts');
}
...
export function arrangeSession({
  ...
  const { turn, path: versionPath } = allocateTurn(dir);
