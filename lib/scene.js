...
const STICKY_CHARS_PER_LINE = 34;   // rough wrap estimate at fontSize 16 in Virgil
const PADDING = 14;
...
export function buildSticky({ tone, text, x = 0, y = 0, groupId }) {
  ...
  const w = STICKY_W;
  const lines = estimateLines(text, STICKY_CHARS_PER_LINE);
  ...
  const textEl = baseElement({
    ...
    text,
    fontSize: 16,
    fontFamily: 2,  // Helvetica: guaranteed to load (Virgil can flicker-hide)
    textAlign: 'left',
    verticalAlign: 'top',
    baseline: 18,
    containerId: rect.id,
...
export function buildPanel({ title, body, x = 0, y = 0, groupId }) {
  ...
  const bodyCharsPerLine = 42;
  const lines = body ? body.split('\n').reduce(
    (acc, ln) => acc + Math.max(1, Math.ceil((ln.length || 1) / bodyCharsPerLine)), 0) : 1;
  ...
  const titleEl = baseElement({
    ...
    fontSize: 18, fontFamily: 2, // Helvetica: always available, avoids Virgil font-load flicker
    strokeColor: TEXT_COLOR,
