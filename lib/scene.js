...
export function buildSticky({ tone, text, x = 0, y = 0, groupId }) {
  ...
  // Free-floating text inside the rectangle bounds (like panels).
  // Bound text triggers Excalidraw autoresize that clips / re-measures
  // after updateScene and causes blank or overflowing text.
  const textEl = baseElement({
    type: 'text',
    ...
    verticalAlign: 'top',
    baseline: 18,
    autoResize: false,     // wrap at fixed width
    strokeColor: TEXT_COLOR,
    groupIds: [gid],
  });
  rect.boundElements = [];
